      ...
      .then(({ data }) => {
        if (data.status === "exists") {
          ...
          showNotification({
            ...
            text: "Подтвердите свой аккаунт перейдя по ссылке в письме которое было отправлено на указанный адрес электронной почты (не забудьте проверить папку Спам)",
            type: "success",
            duration: 0,
          });
          setSubmitting(false);
